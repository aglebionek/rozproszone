...
class Window:
    ...
    def generate_create_game_frame(self):
        ...
        self.create_game_errors = Text(master=self.create_game_main_frame, fg="red", state="disabled")
    ...
    def generate_game_frame(self):
        ...
        # button controls
        Button(master=self.game_main_frame, text='Rock', command=lambda: self.actions.make_move_action("rock"), width=33).grid(row=3, column=0, padx=2, pady=5)
        Button(master=self.game_main_frame, text='Paper', command=lambda: self.actions.make_move_action("paper"), width=33).grid(row=3, column=1, columnspan=2, padx=2, pady=5)
        Button(master=self.game_main_frame, text='Scisors', command=lambda: self.actions.make_move_action("scissors"), width=33).grid(row=3, column=3, padx=2, pady=5)
